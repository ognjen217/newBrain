...
class NucleoComm:
    ...
    def send_command(self, command, response_wait=1.0):
        """
        Sends a command via SerialHandler and returns any responses (parsed).
        :param command: Command string (e.g., "SPEED:10").
        :param response_wait: Time to wait for responses.
        :return: List of dictionaries, e.g. [{"action": "speed", "value": 100.0, "raw": "@speed:100;;"}]
        """
        if self.serial_handler is None:
            self.logger.error("Serial handler not available.")
            return None
        try:
            # Očistimo staru bufferiranu listu:
            self.serial_handler.get_messages()
            # Pošaljemo novu komandu
            self.serial_handler.send_command(command)
            self.logger.info("Command sent: %s", command)
            # Pričekamo malo na odgovor
            time.sleep(response_wait)
            # Pokupimo što je stiglo (parsirane poruke)
